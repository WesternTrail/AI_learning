...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def max_by_axis(self, the_list): # 活的多张图片中的最大通道，最大宽，最大高
        ...
        for sublist in the_list[1:]:
            for index, item in enumerate(sublist):
                maxes[index] = max(maxes[index], item) # maxes[0]存储最大通道数，[1]存储最大的高，[2]最大的宽
        return maxes
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        if torchvision._is_tracing():
            ...
            return self._onnx_batch_images(images, size_divisible) # 针对GPU进一步优化

        # 分别计算一个batch中所有图片中的最大channel, height, width
        max_size = self.max_by_axis([list(img.shape) for img in images])

        stride = float(size_divisible) # 将刚刚获得的最大的边长对向上取整到32的倍率值
        # max_size = list(max_size)
        # 将height向上调整到stride的整数倍
        max_size[1] = int(math.ceil(float(max_size[1]) / stride) * stride) # 将刚刚获得的最大的边长对向上取整到32的倍率值
    ...
    def forward(self,
        ...
        for i in range(len(images)):
            image = images[i]
            target_dict = targets[i] if targets is not None else None
        ...
        for image_size in image_sizes: # 记录resize之后的尺寸,并判断是否合法
            assert len(image_size) == 2
            image_sizes_list.append((image_size[0], image_size[1]))

        image_list = ImageList(images, image_sizes_list) # 绘制bbox的时候要在原图上绘制
