...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    box_loss = det_utils.smooth_l1_loss(
        ...
        box_regression[sampled_pos_inds_subset, labels_pos], # 取正样本预测的那个类的回归参数
        regression_targets[sampled_pos_inds_subset], # 真实回归参数
        beta=1 / 9,
        size_average=False,
    ) / labels.numel()
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
                 box_roi_pool,   # Multi-scale RoIAlign pooling,当成黑盒直接用
                 box_head,       # TwoMLPHead
                 box_predictor,  # FastRCNNPredictor
                 # Faster R-CNN training
                 fg_iou_thresh, bg_iou_thresh,  # default: 0.5, 0.5,训练faster-rcnn时要用到
        ...
        self.box_coder = det_utils.BoxCoder(bbox_reg_weights) # 不知道这个干啥用
        ...
        self.score_thresh = score_thresh  # default: 0.05，移除低概率目标
        self.nms_thresh = nms_thresh      # default: 0.5 ROI_head得到预测框后的nms阈值
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            matched_idxs.append(clamped_matched_idxs_in_image) # 暂时把所有的背景和丢弃的样本分配给0号GT
            labels.append(labels_in_image)    # 每个proposal所对应的类别(正样本(>=1)，负样本，丢弃样本)
        return matched_idxs, labels

    def subsample(self, labels):
        # type: (List[Tensor]) -> List[Tensor]
        # BalancedPositiveNegativeSampler
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels) # 返回的都是正负样本蒙版
        sampled_inds = []
        # 遍历每张图片的正负样本索引
    ...
    def add_gt_proposals(self, proposals, gt_boxes):
        ...
        """
        proposals = [ # 因为正样本太少，将GTBOX也一起放到样本中训练
    ...
    def postprocess_detections(self,
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            labels = labels.view(1, -1).expand_as(scores) # 首先新增加一个维度，然后再进行扩充，不是用的scores扩充而是0,1,2,3
            ...
            keep = keep[:self.detection_per_img] # 取前100个高分的
            boxes, scores, labels = boxes[keep], scores[keep], labels[keep]
