...
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_idxs):
        ...
        neg_idx = []
        # 遍历每张图像的matched_idxs
        for matched_idxs_per_image in matched_idxs: # match_idx是匹配标签
            # >= 1的位置为正样本, nonzero返回非零元素索引
            # positive = torch.nonzero(matched_idxs_per_image >= 1).squeeze(1)
            positive = torch.where(torch.ge(matched_idxs_per_image, 1))[0] # 返回所处的位置
            # = 0的为负样本
            ...
            # 随机选择指定数量的正负样本,负样本有很多，从中选一点，但是正样本一般都会选到
...
class BoxCoder(object):
    ...
    def decode(self, rel_codes, boxes):
        ...
        concat_boxes = torch.cat(boxes, dim=0) # 把一个batch_size中生成的所有anchor进行cat

        box_sum = 0
        for val in boxes_per_image:
            box_sum += val

        # 将预测的bbox回归参数应用到对应anchors上得到预测bbox的坐标,左上，右下
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        return pred_boxes # proposal（xmin,ymin,xmax,ymax）
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        # 计算iou小于low_threshold的蒙版
        below_low_threshold = matched_vals < self.low_threshold
        # 计算iou在low_threshold与high_threshold之间的蒙版
