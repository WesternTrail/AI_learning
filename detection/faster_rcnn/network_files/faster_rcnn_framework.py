...
class FasterRCNNBase(nn.Module):
    ...
    Arguments:
        ...
        roi_heads (nn.Module): takes the features + the proposals from the RPN and computes
            detections / masks from it. ROIpooling一直到postprocess detection这一部分都是
    ...
    def __init__(self, backbone, rpn, roi_heads, transform):
        ...
        self.transform = transform # 是对图像进行标准化，规定最小最大的尺寸
    ...
    def forward(self, images, targets=None):
        ...
        """
        if self.training and targets is None:  # 训练阶段时样本没有框报错
        ...
        if self.training:
            ...
            for target in targets:  # 进一步判断传入的target的boxes参数是否符合规定
                ...
                if isinstance(boxes, torch.Tensor):
                    if len(boxes.shape) != 2 or boxes.shape[-1] != 4:  # 判断boxes是不是n行4列
        ...
        original_image_sizes = torch.jit.annotate(List[Tuple[int, int]],
                                                  [])  # 提示original_image_sizes是元素为(int,int)元祖组成的列表？
        for img in images:
            val = img.shape[-2:]  # 获取到高宽
        ...
        if torch.jit.is_scripting():  # 转成torch_script可以优化模型
            ...
            return self.eager_outputs(losses, detections)  # 如果是测试则只返回detections，否则都返回
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 # Box parameters
                 box_roi_pool=None, box_head=None, box_predictor=None,  # 分别对应roi pooling，TO MLP header，对应类别概率边界框回归参数
                 # 移除低目标概率      fast rcnn中进行nms处理的阈值   对预测结果根据score排序取前100个目标
                 box_score_thresh=0.05, box_nms_thresh=0.5, box_detections_per_img=100,
                 box_fg_iou_thresh=0.5, box_bg_iou_thresh=0.5,  # fast rcnn计算误差时，采集正负样本设置的阈值
                 box_batch_size_per_image=512, box_positive_fraction=0.25,  # fast rcnn计算误差时采样的样本数，以及正样本占所有样本的比例
                 bbox_reg_weights=None):
        if not hasattr(backbone, "out_channels"): # backbone是否具有out_channels
            raise ValueError(
        ...
        # 若anchor生成器为空，则自动生成针对resnet50_fpn的anchor生成器,mobilenet代码已经创建了
        ...
        # 生成RPN通过滑动窗口预测网络部分,分类和回归
        if rpn_head is None:
            rpn_head = RPNHead( # 第二个参数是每个特征层上的anchor模板个数
        ...
        if box_head is None:
            ...
            box_head = TwoMLPHead(
                ...
                representation_size  # 全连接层的输出节点个数
            )
        ...
        # 数据进行标准化，缩放，打包成batch等处理部分
        transform = GeneralizedRCNNTransform(min_size, max_size, image_mean, image_std)

        super(FasterRCNN, self).__init__(backbone, rpn, roi_heads, transform) # 初始化父类
