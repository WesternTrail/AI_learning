...
class IntermediateLayerGetter(nn.ModuleDict):
    ...
    def __init__(self, model: nn.Module, return_layers: Dict[str, str]) -> None:
        # 看layer3，layer4是否在模型结构中
        if not set(return_layers).issubset([name for name, _ in model.named_children()]):
            raise ValueError("return_layers are not present in model")
        orig_return_layers = return_layers
        return_layers = {str(k): str(v) for k, v in return_layers.items()}

        # 重新构建backbone，将原来的resnet没有使用到的模块全部删掉,avgpooling和fc，直到把layer3,layer4都拿出来
        layers = OrderedDict()
...
def fcn_resnet50(aux, num_classes=21, pretrain_backbone=False):
    ...
    backbone = resnet50(replace_stride_with_dilation=[False, True, True]) # 膨胀卷积

    if pretrain_backbone:
        # 载入resnet50 backbone预训练权重
        backbone.load_state_dict(torch.load("resnet50.pth", map_location='cpu'))

    out_inplanes = 2048 #layer4的输出:(60,60,2048)
    aux_inplanes = 1024 # layer3的输出（60,60,1024）
